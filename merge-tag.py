...
def get_revision(path):
    if path in separated_techpacks:
...
for path, url in modules.items():
    ...
    if path in ['fw-api', 'qca-wifi-host-cmn', 'qcacld-3.0']:
        commit_msg = f"{path.replace('/', ': ')}: Merge tag '{display_rev}'"
        subtree = '-Xsubtree=drivers/staging/' + path
    else:
        commit_msg = (
            f"techpack: {path.replace('/', ': ')}: Merge tag '{display_rev}'"
        )
        subtree = '-Xsubtree=techpack/' + path
